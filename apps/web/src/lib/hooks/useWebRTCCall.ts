...
import type { CallSignal } from '../services/CallSignalingService'
...
export function useWebRTCCall(
  ...
  const signalingRef = useRef<CallSignalingService | null>(null)
  ...
  useEffect(() => {
    if (!myUserId) return
    signalingRef.current = new CallSignalingService(myUserId)

    const ch = signalingRef.current.listenForIncomingCalls((signal) => {
